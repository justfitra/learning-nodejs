...
const create = async (req, res, next) => {
  try {
    const users = await userService.create(userRepository);
...
const update = async (req, res, next) => {
  try {
    const user = await userService.update(userRepository, req, req.params);
...
const del = async (req, res, next) => {
  try {
    const user = await userService.del(userRepository, req.params);

    return req.status(200).json(formatResponse(200, "Success"));
  } catch (err) {
    next(err);
  }
};
